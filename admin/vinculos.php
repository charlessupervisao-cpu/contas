      ...
      <div class="field">
        ...
        <select class="select" name="exp_<?= e($code) ?>" <?= can_launch($user['role'])?'':'disabled' ?>>
          ...
          <?php foreach ($accounts as $i => $a): ?>
            <option value="<?= e($a['id']) ?>" <?= ($map['EXPENSE_CATEGORY|'.$code] ?? '') === $a['id'] ? 'selected' : '' ?>>Conta <?= $i+1 ?> — <?= e($a['label']) ?></option>
          <?php endforeach; ?>
        </select>
      </div>
    <?php endforeach; ?>
    ...
    <?php foreach (REVENUE_SOURCES as $code => $label): ?>
      <div class="field">
        ...
        <select class="select" name="rev_<?= e($code) ?>" <?= can_launch($user['role'])?'':'disabled' ?>>
          <option value="">— sem vínculo —</option>
          <?php foreach ($accounts as $i => $a): ?>
            <option value="<?= e($a['id']) ?>" <?= ($map['REVENUE_SOURCE|'.$code] ?? '') === $a['id'] ? 'selected' : '' ?>>Conta <?= $i+1 ?> — <?= e($a['label']) ?></option>
          <?php endforeach; ?>
        </select>
      </div>
    <?php endforeach; ?>
